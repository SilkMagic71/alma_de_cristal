...
@onready var musicbar = %General_HScrollBar

@onready var audio_player: AudioStreamPlayer = get_node("%audio_player")
...
func play_sfx_for_regular_button():
	audio_player.stream = load("res://assets/audio/sfx/press_regular_button.mp3")
	audio_player.play()
